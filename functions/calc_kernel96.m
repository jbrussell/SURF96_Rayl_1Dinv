function [dcdvs, dcdvp, dudvs, dudvp, z_mid] = calc_kernel96(model, vec_T, wavetype, ifnorm, ifplot)
...
for ip = 1:Nper
    ...
    make_par_surf96(wavetype); % make a control file for surf96
    % ================2. make model file and dispersion file ================
    ...
    if(wavetype=='J'); % if wavetype is joint L and R, write both to dispersion file, otherwise only write one
        ...
        writedisp_surf96(fakedata,'disp_obs.dsp','L','U');
        writedisp_surf96(fakedata,'disp_obs.dsp','R','U',0);
    else
        writedisp_surf96(fakedata,'disp_obs.dsp',wavetype,'U');
    ...
    kernel= read_kernel96('srfker96.txt',wavetype);
    if isempty(kernel)
