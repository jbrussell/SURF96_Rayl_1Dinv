function [freq,hvsr,vel] = calc_HVf_SW(model, nf,fmin,fmax,nmr,nml)
% Calculate HVSR considering surface waves only
%
% Usage: HV [OPTIONS]
% 
...
[~,log] = system(['HVf -f model.txt -nf ',num2str(nf),' -fmin ',num2str(fmin),' -fmax ',num2str(fmax),' -nmr ',num2str(nmr),' -nml ',num2str(nml),' -logsam -ph -gr -hv  > HV.dat']);
