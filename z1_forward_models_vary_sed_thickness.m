...
% Sediment properties
v_zsed = [0.5 0.75 1.0 1.5 2.0]; %0.25; % depth of base of sediment (i.e., sediment thickness)
vs_sed = 1.0; %
rho_sed = 1.8;
...
%%
N = 1;
clrs_mod = flipud(brewermap(length(v_zsed)+N,'Greys'));
clrs_mod = clrs_mod(1:end-N,:);
clrs_hvsr = flipud(brewermap(length(v_zsed)+N,'Greens'));
clrs_hvsr = clrs_hvsr(1:end-N,:);
clrs_ellip = flipud(brewermap(length(v_zsed)+N,'Blues'));
clrs_ellip = clrs_ellip(1:end-N,:);
clrs_phv = flipud(brewermap(length(v_zsed)+N,'Reds'));
clrs_phv = clrs_phv(1:end-N,:);
clrs_grv = flipud(brewermap(length(v_zsed)+N,'Purples'));
clrs_grv = clrs_grv(1:end-N,:);

lw = 4;
for ii = 1:length(v_zsed)
    
    zsed = v_zsed(ii);
    disp(['zsed = ',num2str(zsed),' km'])
...
% save2pdf('./figs/z1_vary_sed_thickness.pdf',1,300);
